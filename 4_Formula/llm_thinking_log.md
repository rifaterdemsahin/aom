...
### 📥 Input / Task
Refactor `aom` onto https://github.com/rifaterdemsahin/delivery-pilot-template. First logical change: copy the template skeleton and move illegal root extras.

### 💭 Thinking & Reasoning Process
- Confirmed RULE-005 allowed root folders only; `images/` and `.nojekyll` were extras.
- GitHub Actions Pages (`static.yml`) does not need `.nojekyll`.
- Concept photos belong in `5_Symbols/assets/images/` (implementation assets). Simulation will reference them later.
- Kept pre-refactor `index.html` / `README.md` under `5_Symbols/_obsolete/` so the explainer copy is not lost when the template overwrote root files.
- Did not replace placeholders yet (next commit) so this commit is one logical change: skeleton + layout.

### 📤 Outcomes & Decisions
- Template files are in the repo.
- Images moved off the root.
- SPEC-014 drafted as the refactor tracker.
